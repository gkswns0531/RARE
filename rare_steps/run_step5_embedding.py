...
def main():
    ...
    for chunk_id, atomic_list in atomic_info_data.items():
        ...
        for atomic_dict in atomic_list:
            atomic_info = AtomicInfo(
                atomic_info_id=atomic_dict["atomic_info_id"],
    ...
    cost_info = embeddings_data.get('cost_info', {})
    cost = cost_info.get('total_cost_usd', 0.0)
    print(f"Generated embeddings for {total_items} atomic items. Cost: ${cost:.6f}")
